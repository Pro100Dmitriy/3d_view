export function template( data ){
    const title = data.name.replace( /(?<other>[\sа-яА-Я0-9a-zA-Z&<br>]+)\[i\](?<title>\w+)\[-i\]/gu, ( ...match ) => {
        let groups = match.pop()
        return `${groups.other} <span style="color:${data.color};">${groups.title}</span>`
    } )
